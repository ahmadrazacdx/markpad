...
fn select_backend_port() -> Result<u16, String> {
    let preferred_port = env::var("MARKTEX_BACKEND_PORT")
        .ok()
...
fn spawn_backend(app: &tauri::AppHandle, port: u16) -> Result<Child, String> {
    ...
    let renderer_warmup_delay_ms =
        env::var("MARKTEX_RENDERER_WARMUP_DELAY_MS").unwrap_or_else(|_| "0".to_string());
    ...
    command
        ...
        .env("MARKTEX_RENDERER_WARMUP_DELAY_MS", renderer_warmup_delay_ms)
        .env("MARKTEX_DATA_DIR", &app_storage_dir)
        .env("MARKTEX_PANDOC_BIN", &pandoc_bin)
        .env("MARKTEX_TYPST_BIN", &typst_bin)
        .env("MARKTEX_PDF_DISABLE_LATEX", "1")
